...
fn move_players_system(
  ...
  for (mut linear_velocity, mut view_direction, input) in query.iter_mut() {
    ...
    // move by x axis
    linear_velocity.x += dx * PLAYER_MOVE_SPEED * global_x.x * 1.5_f32.powf(input.sprint as i32 as f32); // * time.delta().as_secs_f32();
    linear_velocity.z += dx * PLAYER_MOVE_SPEED * global_x.z * 1.5_f32.powf(input.sprint as i32 as f32); // * time.delta().as_secs_f32();
