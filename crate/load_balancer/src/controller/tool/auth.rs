    ...
    match token {
      Some(token) => {
        ...
        if let Ok(claims) = verify_token(token) {
          Box::pin(async move { Outcome::Success(TokenHeader { id: claims.sub }) })
        } else {
          Box::pin(async { Outcome::Failure((Status::Unauthorized, ApiTokenError::Invalid)) })
        }
      }
      None => Box::pin(async { Outcome::Failure((Status::Unauthorized, ApiTokenError::Missing)) }),
    }
  }
}
